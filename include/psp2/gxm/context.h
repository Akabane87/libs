...
#ifndef _PSP2_GXM_CONTEXT_H_
#define _PSP2_GXM_CONTEXT_H_
 ...
 * <c><i>minSize</i></c> should always be at least
 * #SCE_GXM_MINIMUM_DEFERRED_CONTEXT_BUFFER_SIZE bytes in size.
 * 
 ...
 * to be used without clipping or viewport transformation taking place when disabled.
 * 
...
/** 
 * Blocks the GPU from consuming further draw calls until all pending 
 * memory transactions from the vertex programs of any preceding draw 
 ...
 * #SCE_GXM_ERROR_NOT_WITHIN_COMMAND_LIST error code being returned.
 * 
 * It is only necessary to call this function in situations where data 
 * is written to writable uniform buffers by the vertex programs of one 
 * or more draw calls and the same data is required by subsequent draw 
 * calls within the same vertex job. In such situations 
 * #sceGxmVertexFence() should be called ensuring that draw calls which 
 * come after #sceGxmVertexFence() are consuming data that is written by 
 * those draw calls which come before. 
 * 
 * @param[in,out]	context			A pointer to the rendering context.
 * 
 * @retval
 ...
 * @retval
 * SCE_GXM_ERROR_RESERVE_FAILED The operation failed due to a buffer callback function
 * failing to provide sufficient memory for GPU data structures for the fence.
 * This error will only be returned from a deferred context. Immediate context reservations
 * always succeed,	but they potentially have high latency if scenes are split to recycle memory.
 * 
 * @ingroup render
 */
SceGxmErrorCode sceGxmVertexFence(
	SceGxmContext *context);
